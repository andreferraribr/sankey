...
influx_s<- influx%>%
  filter(ano == "2018", influx_reais>10000000000, ind_pri_cod == "P")%>%
  group_by(esfera_in,origem_cod,fonte)%>%
...
# filtra nodes objeto do estudo
outflux_s<- outflux%>%
  filter(ano == "2018", outflux_reais>100000000, esfera_out_cod == "S", gnd_cod %in% c(3), elemento_cod != 81 )%>%
  group_by(fonte,esfera_out, elemento)%>%
  summarise(value = sum(outflux_reais/1000000000))
names<- c()
