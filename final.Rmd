...
format_real(nat_rec$influx_reais)
...
format_real(outflux$outflux_reais)
...
cols<- c("DRU" = "blue",  "Recursos Ordinários" = "#ff0000")
...
p <- plot_ly(dru_ord)%>%
  ...
  geom_text(aes(label=value), size= 3 ,vjust=-0.25)
...
cols<- c("DRU" = "blue",  "REMUNERACAO DAS DISPONIB. DO TESOURO NACIONAL" = "#4d4d4d", "TITULOS DE RESPONSABILID.DO TESOURO NACIONAL" = "#b3b3b3", "RECURSOS ORDINARIOS" = "#ff0000")
...
ggplot( aes(x = ano, y = value)) +
  ...
  geom_line(group=1)+
  theme_bw()+
  ggtitle("DRU x recursos destinados à seguridade social", "Governo Federal 2005 a 2018 (R$ bi nominais)")+
  xlab("")+
  ylab("")+
  theme(
    panel.grid.major.x = element_blank(),
