...
fluxo<- fluxo%>%
  mutate(tipo = ifelse(value<0, "DRU", "fonte_100"))%>%
  arrange((tipo))


ggplot(dru_ord, aes(x = ano, y = value)) +
...
# recursos ordinários utilizados, remuneração conta única, concessões e títulos para despesas com o orçamento da seguridade social
ord_1<-outflux%>%
  filter(fonte_cod %in% c("100","188","144","329"), esfera_out_cod == "S" )
...
dru_ord_1<- merge(ord_ano_1, dru_ano_1, by.x = "ano", by.y = "ano")
...
fluxo_1<- fluxo_1%>%
  mutate(tipo = ifelse(value>0, "DRU", "fonte_fiscais"))%>%
  arrange((tipo))


ggplot(dru_ord_1, aes(x = ano, y = value)) +
