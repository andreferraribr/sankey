...
ord_ano<- ord_ano%>%
  mutate (tipo = "retorno_via_fonte_100")

plot_in_out<- rbind(dru_ano,ord_ano)%>%
  arrange(desc(ano ))




the_order <-dru_ano$ano



p <- 
  plot_in_out %>% 
  ggplot(aes(x = ano, y = value, group = , fill = tipo)) +
  geom_bar(stat = "identity", width = 0.75) +
...
ggplot(data = dru_ord, aes(x = ano, y = resultado))+
  geom_point()
...
dru_ord<- dru_ord %>%  
  mutate (resultado = value.x + value.y  )
